Feature: The application's GET User Data endpoint
  ...
  Scenario: The backend is able to process User Data with userId GET requests
    ...
    When I GET "/audit/userData" using query param either userId "13e31622-edea-493c-8240-9b780c9d6111" or email ""
  ...
  Scenario: The backend is able to process User Data with emailId GET requests
    Given LAU EUD backend application is healthy
    When I GET "/audit/userData" using query param either userId "" or email "john111.smith111@example.org"
  ...
  Scenario: The backend is unable to process userData GET requests due to missing s2s
    Given LAU EUD backend application is healthy
    When I GET "/audit/userData" without service authorization header
  ...
  Scenario: The backend is unable to process userData GET requests due to invalid s2s
    Given LAU EUD backend application is healthy
    When I GET "/audit/userData" using query param userId "13e31622-edea-493c-8240-9b780c9d6111" with invalid service authorization header
    Then HTTP "403" Forbidden response is returned
